...
function resolveTier(total, maxScore) {
  ...
  if (r < 0.34) {
    return {
      label: '筛查倾向：较低',
      desc: "在寻求关注、戏剧化表达与情绪外显相关条目上得分整体偏低。15 题仅为粗筛，不构成表演型人格障碍或任何精神障碍诊断。",
    };
  }
  if (r < 0.62) {
    return {
      label: '筛查倾向：中等',
      desc: "你可能在社交中较热情或偶尔夸张。若「必须被看见」已影响关系稳定或自我感受，建议与心理咨询师讨论；本结果不能替代专业评估。",
    };
  }
  return {
    label: '筛查倾向：偏高',
    desc: "条目提示在「需要关注、关系亲密度判断、戏剧化反应」等维度上体验较明显。这不等于人格障碍诊断；若伴随显著痛苦或功能受损，请寻求专业评估。",
  };
}
...
Page({
  ...
  applyLocalQuestions() {
    try {
      const pack = require('./hpd-questions.json');
  ...
  persistAndGoResult({ totalScore, maxScore, tier }) {
    const ts = Date.now();
    let history = wx.getStorageSync("hpdHistory") || [];
    ...
    wx.setStorageSync("hpdHistory", history.slice(0, 20));
    wx.setStorageSync("hpdLastResultView", {
      totalScore,
      maxScore,
    ...
    wx.redirectTo({ url: "/pages/hpd/result/index" });
  },
});
