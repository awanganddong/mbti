...
Page({
  data: {
    ...
    currentQuestion: personalityTest[0],
  ...
  setQuestion(questionIndex, userAnswers) {
    const total = personalityTest.length;
    const percent = total <= 1 ? 100 : Math.round((questionIndex / (total - 1)) * 100);
    this.setData({
      currentQuestionIndex: questionIndex,
      currentQuestion: personalityTest[questionIndex],
  ...
  previousQuestion() {
    if (this.data.currentQuestionIndex > 0) {
      ...
      this.setQuestion(newIndex, this.data.userAnswers);
    }
  },
  ...
  nextQuestion() {
    ...
    newAnswers[this.data.currentQuestionIndex] = this.data.selectedAnswer;

    if (this.data.currentQuestionIndex === personalityTest.length - 1) {
      // 测试完成，计算结果
      this.calculateResult(newAnswers);
    } else {
      // 下一题
      const newIndex = this.data.currentQuestionIndex + 1;
      this.setQuestion(newIndex, newAnswers);
    }
  },
  ...
  calculateResult(answers) {
    ...
    answers.forEach((answer, index) => {
      const question = personalityTest[index];
      const selectedOption = question.answerOptions.find(opt => opt.type === answer);
      if (selectedOption) {
    ...
    // 导航到结果页面
    wx.navigateTo({
      url: `/pages/test/result/index?type=${personalityType}&testResultId=${timestamp}`
    });
  }
})
