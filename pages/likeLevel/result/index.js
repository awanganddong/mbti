...
Page({
  ...
  onShareAppMessage() {
    ...
    const title = `我的喜欢程度：${this.data.resultTitle}（${this.data.score}/${this.data.maxScore}）`;
    return {
      title,
      path: testResultId ? `/pages/likeLevel/result/index?testResultId=${testResultId}` : '/pages/likeLevel/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `喜欢程度：${this.data.resultTitle} ${this.data.score}/${this.data.maxScore}`;
    return {
      title,
