...
Page({
  ...
  onShareAppMessage() {
    ...
    const title = `我的直男指数：${this.data.score}/${this.data.maxScore}`;
    return {
      title,
      path: testResultId ? `/pages/man/result/index?testResultId=${testResultId}` : '/pages/man/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `直男指数：${this.data.score}/${this.data.maxScore}`;
    return {
      title,
