import { animalSuuTest } from '../../../data/animal';

Page({
  data: {
    score: 0,
    maxScore: animalSuuTest.maxScore,
    resultTitle: '',
  ...
  onLoad(options) {
    ...
    if (!record) {
      const fallback = animalSuuTest.results.cat;
      this.setData({
        score: 0,
  ...
  onShareAppMessage() {
    ...
    const title = `我的动物塑：${this.data.resultTitle}（主塑 ${this.data.score}/${this.data.maxScore}）`;
    return {
      title,
      path: testResultId ? `/pages/animal/result/index?testResultId=${testResultId}` : '/pages/animal/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `动物塑：${this.data.resultTitle} ${this.data.score}/${this.data.maxScore}`;
    return {
      title,
