import { idealPartnerTest } from '../../../data/ideal';

Page({
  data: {
    score: 0,
    maxScore: idealPartnerTest.maxScore,
    resultTitle: '',
  ...
  onLoad(options) {
    ...
    if (!record) {
      const fallback = idealPartnerTest.results.gentle;
      this.setData({
        score: 0,
  ...
  onShareAppMessage() {
    ...
    const title = `我的理想恋人类型：${this.data.resultTitle}（${this.data.score}/${this.data.maxScore}）`;
    return {
      title,
      path: testResultId ? `/pages/ideal/result/index?testResultId=${testResultId}` : '/pages/ideal/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `理想恋人：${this.data.resultTitle} ${this.data.score}/${this.data.maxScore}`;
    return {
      title,
