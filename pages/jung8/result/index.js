Page({
  data: {
    topTwoLabel: '',
    typeGuess: '',
    resultTitle: '',
    resultDescriptionParagraphs: [],
    sortedScores: [],
    currentTestResultId: '',
  },

  onLoad(options) {
    wx.showShareMenu({
      withShareTicket: true,
      menus: ['shareAppMessage', 'shareTimeline']
    });
    const testResultId = options.testResultId;
    this.setData({
      currentTestResultId: testResultId || ''
    });
    const history = wx.getStorageSync('jung8TestHistory') || [];
    let record = null;
    if (testResultId) {
      record = history.find(item => item.timestamp.toString() === testResultId) || null;
    } else {
      record = history.length > 0 ? history[0] : null;
    }

    if (!record) {
      this.setData({
        topTwoLabel: '—',
        typeGuess: '',
        resultTitle: '暂无测评记录',
        resultDescriptionParagraphs: ['请先完成荣格八维测试。'],
        sortedScores: [],
      });
      return;
    }

    this.setData({
      topTwoLabel: record.topTwoLabel || '—',
      typeGuess: record.typeGuess || '',
      resultTitle: record.resultTitle || '荣格八维排序',
      resultDescriptionParagraphs: this.splitIntoParagraphs(record.resultDescription),
      sortedScores: record.sortedScores || [],
      currentTestResultId: record.timestamp ? String(record.timestamp) : (testResultId || ''),
    });
  },

  onShareAppMessage() {
    const testResultId = this.data.currentTestResultId;
    const title = `我的荣格八维：${this.data.topTwoLabel}${this.data.typeGuess ? ' · 参考 ' + this.data.typeGuess : ''}`;
    return {
      title,
      path: testResultId ? `/pages/jung8/result/index?testResultId=${testResultId}` : '/pages/jung8/index',
      imageUrl: '/public/images/mbti.jpg'
    };
  },

  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `荣格八维：${this.data.topTwoLabel}`;
    return {
      title,
      query: testResultId ? `testResultId=${testResultId}` : '',
      imageUrl: '/public/images/mbti.jpg'
    };
  },

  shareTimelineTip() {
    wx.showToast({
      title: '点击右上角 ··· 分享到朋友圈',
      icon: 'none'
    });
  },

  splitIntoParagraphs(text) {
    if (!text) return [];
    const normalized = String(text).replace(/\s+/g, ' ').trim();
    if (!normalized) return [];
    const parts = [];
    let current = '';
    for (let i = 0; i < normalized.length; i++) {
      const ch = normalized[i];
      current += ch;
      if (ch === '。' || ch === '！' || ch === '？' || ch === '.' || ch === '!' || ch === '?') {
        const trimmed = current.trim();
        if (trimmed) parts.push(trimmed);
        current = '';
      }
    }
    const tail = current.trim();
    if (tail) parts.push(tail);
    return parts;
  },

  goHome() {
    wx.navigateTo({
      url: '/pages/index/index'
    });
  },

  takeTestAgain() {
    wx.navigateTo({
      url: '/pages/jung8/index'
    });
  }
});
