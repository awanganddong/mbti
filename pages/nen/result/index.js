...
Page({
  ...
  onShareAppMessage() {
    ...
    const title = `我的年上年下倾向：${this.data.resultTitle}（${this.data.score}/${this.data.maxScore}）`;
    return {
      title,
      path: testResultId ? `/pages/nen/result/index?testResultId=${testResultId}` : '/pages/nen/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `年上年下倾向：${this.data.resultTitle} ${this.data.score}/${this.data.maxScore}`;
    return {
      title,
