import { loveQuirkTest } from '../../../data/loveQuirk';

Page({
  data: {
    score: 0,
    maxScore: loveQuirkTest.maxScore,
    resultTitle: '',
  ...
  onLoad(options) {
    ...
    if (!record) {
      const fallback = loveQuirkTest.results.clingy;
      this.setData({
        score: 0,
  ...
  onShareAppMessage() {
    ...
    const title = `我的恋爱癖好：${this.data.resultTitle}（${this.data.score}/${this.data.maxScore}）`;
    return {
      title,
      path: testResultId ? `/pages/lovequirk/result/index?testResultId=${testResultId}` : '/pages/lovequirk/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `恋爱癖好：${this.data.resultTitle} ${this.data.score}/${this.data.maxScore}`;
    return {
      title,
