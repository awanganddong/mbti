import { plantSuuTest } from '../../../data/plant';

Page({
  data: {
    score: 0,
    maxScore: plantSuuTest.maxScore,
    resultTitle: '',
  ...
  onLoad(options) {
    ...
    if (!record) {
      const fallback = plantSuuTest.results.cactus;
      this.setData({
        score: 0,
  ...
  onShareAppMessage() {
    ...
    const title = `我的植物塑：${this.data.resultTitle}（主塑 ${this.data.score}/${this.data.maxScore}）`;
    return {
      title,
      path: testResultId ? `/pages/plant/result/index?testResultId=${testResultId}` : '/pages/plant/index',
  ...
  onShareTimeline() {
    const testResultId = this.data.currentTestResultId;
    const title = `植物塑：${this.data.resultTitle} ${this.data.score}/${this.data.maxScore}`;
    return {
      title,
